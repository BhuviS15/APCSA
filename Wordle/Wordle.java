...
public class Wordle
{
    ...
    {
        int row = wordle.getCurrentRow();    // replace this line
    ...
    private boolean findCorrectLetters(String guess)
    {
        ...
        for (int col = 0; col < wordle.N_COLS; col++)
        {
            ...
            String s = secretWord.substring(col, col + 1);
            
            if (g.equals(s))
            {
                wordle.setSquareColor(row, col, wordle.CORRECT_COLOR);
                wordle.setKeyColor(g, wordle.CORRECT_COLOR);
            }
            else {
    ...
    private int presentLetterCount(String letter)
    {
        ...
        while (col >= 0)
        {
           if (wordle.getSquareColor(row,col) != wordle.CORRECT_COLOR)
           {
              count++; 
           }
